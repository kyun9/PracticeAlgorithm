...
public class 백준_주사위윳놀이_17825 {
	...
	static class Node {
		...
		Node addNext(int value) {					//노드 이어가기
		...
		static Node getNode(int target) {		//원하는 노드 찾아서 리턴
			Node temp = start;
	...
	public static void main(String[] args) throws IOException {
		...
		selections = new int[10];	// 10번의 주사위를 굴릴떄 어떤 말을 선택할 것인가?에 대한 배열
		dice = new int[10];			//주사위 경우
		markers = new Node[4];	// 말4개
	...
	private static void permutation(int depth) {
		if (depth == 10) {
			...
			int score = play();
			ans = score > ans ? score : ans;	//최대값 초기화
